...
import type { frameType } from '@/types/frames/index';
...
const handleAddClick = () => {
    ...
    newReservation.value = { 
        ...
        id: 0,
        index: 0,
        clinic: Number(frameInfo.value?.clinic),
        room: 0,
        doctor: 0,
        date: '',
        fromTime: '',
...
const handleEditClick = (data: reservationType, index: number) => {
...
const fetchReservations = async (filters: filterOptionType[] = []) => {
    try {
        const response = await axios.get('/reservations/frame', {
            params: { id: frameId }
        });
        ...
        frameInfo.value = response.data.frame;
...
const saveReservation = handleSubmit(async (values) => {
    ...
    if (isEdit.value) {
        ...
        try {
            ...
            frameInfo.value.remainReserve -= 1;
                ...
                <v-card elevation="10" class="">
                    ...
                    <div class="d-flex justify-space-between">
                        <v-card-title class="text-h5 pt-sm-2 pb-7">
                            予約データ管理 (
                            {{ frameInfo?.id }}, 
                            {{ clinics[frameInfo?.clinic]?.title }}, 
                            {{ doctors[frameInfo?.doctor]?.name }}, 
                            {{ rooms[frameInfo?.room]?.name }}, 
                            {{ clinics[frameInfo?.clinic]?.title }}, 
                            {{ frameInfo?.date }}, 
                            {{ frameInfo?.fromTime.slice(0, 5) }} ~
                            {{ frameInfo?.toTime.slice(0, 5) }}, 
                            {{ frameInfo?.remainReserve }} /
                            {{ frameInfo?.availableReserve }}
                            )
                        </v-card-title>
                        <div>
